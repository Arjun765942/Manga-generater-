                        ...
                        <div className={`p-3 border-4 border-dashed ${props.hero ? 'border-green-500 bg-green-50' : 'border-blue-300 bg-blue-50'} transition-colors relative group`}>
                            ...
                            {props.hero ? (
                                <div className="flex gap-3 items-center mt-1">
                                     ...
                                     <label htmlFor="hero-replace" className="cursor-pointer comic-btn bg-yellow-400 text-black text-sm px-3 py-1 hover:bg-yellow-300 transition-transform active:scale-95 uppercase focus-within:outline focus-within:outline-4 focus-within:outline-yellow-400 focus-within:outline-offset-2">
                                         REPLACE
                                         <input id="hero-replace" type="file" accept="image/*" className="sr-only" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                     </label>
                                </div>
                            ) : (
                                <label htmlFor="hero-upload" className="comic-btn bg-blue-500 text-white text-lg px-3 py-3 block w-full hover:bg-blue-400 cursor-pointer text-center focus-within:outline focus-within:outline-4 focus-within:outline-yellow-400 focus-within:outline-offset-2">
                                    UPLOAD HERO 
                                    <input id="hero-upload" type="file" accept="image/*" className="sr-only" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                </label>
                            )}
                        </div>
                        ...
                        <div className={`p-3 border-4 border-dashed ${props.friend ? 'border-green-500 bg-green-50' : 'border-purple-300 bg-purple-50'} transition-colors`}>
                            ...
                            {props.friend ? (
                                <div className="flex gap-3 items-center mt-1">
                                    ...
                                    <label htmlFor="friend-replace" className="cursor-pointer comic-btn bg-yellow-400 text-black text-sm px-3 py-1 hover:bg-yellow-300 transition-transform active:scale-95 uppercase focus-within:outline focus-within:outline-4 focus-within:outline-yellow-400 focus-within:outline-offset-2">
                                        REPLACE
                                        <input id="friend-replace" type="file" accept="image/*" className="sr-only" onChange={(e) => e.target.files?.[0] && props.onFriendUpload(e.target.files[0])} />
                                    </label>
                                </div>
                            ) : (
                                <label htmlFor="friend-upload" className="comic-btn bg-purple-500 text-white text-lg px-3 py-3 block w-full hover:bg-purple-400 cursor-pointer text-center focus-within:outline focus-within:outline-4 focus-within:outline-yellow-400 focus-within:outline-offset-2">
                                    UPLOAD CO-STAR 
                                    <input id="friend-upload" type="file" accept="image/*" className="sr-only" onChange={(e) => e.target.files?.[0] && props.onFriendUpload(e.target.files[0])} />
                                </label>
                            )}
                        </div>
